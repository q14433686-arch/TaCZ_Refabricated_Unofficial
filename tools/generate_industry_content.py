...
def render_icon_coverage_document(catalog: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "- `needs_art = true` 的每一行都是连已声明后备图都没有的硬缺口。某些真实工业物件可明确接受通用回退，精确/通用数量仍会分列显示。",
        "- 因此这里的 `0 needs_art` 只表示不会紫黑/空白，**不等于所有身份都有型号级精细美术**；",
        "  第三方实体载具的中性/家族复用精细材质目录、中文名称与容量变体见 `THIRD_PARTY_FEED_GAP_REGISTER.md`。",
        "",
        "## 汇总",
        "",
...
def run(write: bool) -> int:
    ...
    english: dict[str, str] = {
        ...
        "commands.tacz.industry.feed_audit": "§bFeed adapter audit§r — accepted: %s, optional-pack dormant: %s, rejected: %s",
    ...
    chinese: dict[str, str] = {
        ...
        "commands.tacz.industry.feed_audit": "§b供弹适配审计§r — 已接受：%s，可选包休眠：%s，被拒绝：%s",
