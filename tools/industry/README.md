...
Cartridge manifests in `tools/industry/cartridges.json` cover every bundled default loose-ammo id (currently 24). They generate case/projectile die calibration, forming, dedicated cartridge-assembly definitions, legacy-ammo replacement declarations, recoverable spent-case reconditioning routes, and all magazine calibration recipes derived from `industry/gun_feed/*.json`. A declared master gun is verified to actually use that ammo id; default ammo with no matching bundled firearm uses an explicitly named multi-slot mechanical-crafter calibre gauge instead of an unrelated gun. The generator rejects a physical-magazine ammo family or bundled default ammo id that has no cartridge source manifest.
