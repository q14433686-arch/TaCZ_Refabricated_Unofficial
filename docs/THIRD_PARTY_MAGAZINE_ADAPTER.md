...
不额外制造一个会绕过真实工作台语义的“搜索方块”。现有 Gunsmith Table 的左侧 `S`（配方检索）会打开真实 GUI 内的检索面板；输入关键词后，右侧列表改为**跨全部已允许页签**的结果，不必逐页翻找。空格分隔的词按 AND 匹配，检索范围包含：

```text
成品显示名
完整 recipe ID / namespace / path
实际页签 ID
成品 GunId / AttachmentId / AmmoId
工业件的平台、部件、口径、弹种、模具目标、蓝图层级
材料原始 item / tag / partial-NBT 文本
```
...
锁定前客户端会核对：查看器类别所属的 table ID 与当前 menu 一致，且 recipe 仍被当前 table 的真实 filter/tab 接受。锁定后可点击工作台左侧的 `×`、任一正常页签，或在配方检索框输入新关键词恢复普通浏览；因此 namespace/手持筛选不会把一个已由查看器选中的合法配方悄悄替换成另一条。

### REI / JEI 枪械供弹查询

REI 与 JEI 新增“**枪械供弹查询 / Gun Feed Reference**”关系页。对枪使用查看配方，或对弹药、已配置实体弹匣/装填器使用查看用途，即可查到：

```text
该枪当前实际接受的散装 AmmoId（含仅在真实实体供弹路径启用的 profile）
→ 已验证的实体弹匣、弹链箱、桥夹、快装器或漏夹样本
→ 对应 GunId
```

页面明确标注“仅供查询，不会消耗物品”。可拆卸/弹链条目展示的是同一运行时兼容判定实际接受的已声明载具容量；固定内仓则明确显示“无可拆卸弹匣”；没有已验证 `gun_feed` 的第三方枪显示“未声明实体供弹装置 / 原有弹药路径”。因此该目录不会从 `reload.type`、枪名、class、模型、容量或同口径猜测出一张弹匣，也不会把 legacy 枪伪装成已经工业化。
