...
2. 没有声明的枪、弹药、配件自动保留原材料表，并在内存中追加通用工业门槛：
   - 枪械：高碳钢板、枪械组件毛坯、黄铜板；
   - 弹药：按原配方成品数量等量追加弹壳毛坯、弹头毛坯、底火、工业推进药；
   - 配件：高碳钢板、黄铜板。

自动替代结果通过现有 `RECIPES` 同步通道传给枪械工作台、JEI 和 REI；不会写入默认枪包，不会要求客户端运行外部脚本。若某平台日后加入手工高保真声明，它会自动优先于通用回退。
...
`industrial_salvage_station` 是一输入、六输出的独立 GUI/红石自动化机器，不把多件回收产物伪装成置物台合成：

- 空的已配置实体弹匣 → `magazine_blank`，可再次由对应成枪的部署器校准；带余弹的弹匣会被拒绝，必须先卸弹；
- 已配置的模具或淬硬口径量规 → 高碳钢板和黄铜粒；
- 由工业终端实际产出的枪（带 `IndustryAssemblyPlatform`） → 三个可重新成型的结构毛坯和一块高碳钢板；枪内置供弹/枪膛的散装弹会安全返还到输出槽。

回收站会拒绝仍装有**实体外部供弹器**或任意玩家安装配件的枪，避免静默销毁弹匣、光学件或枪口件。先正常退匣、拆配件，再送入回收站。普通旧枪、战利品枪或只靠相同 `GunId` 伪造的枪没有工业终端来源标记，不能成为回收工业零件的捷径。

`magazine_pouch` 保存最多四只完整实体弹匣（余弹、口径和兼容族均保留）；手持袋在背包中右击实体弹匣收纳，右击空槽指定取出，或手持袋在空中右击取出一只。`magazine_loader` 是可复用装弹工具：手持它在背包中右击实体弹匣，会从玩家背包一次性转入所有匹配的散装弹。两者不把实体弹匣改回整数计数，也不绕过现有兼容性检查。
