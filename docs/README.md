...
>
> **本分支刻意没有做 26.2 那样的目录迁移**（`investigations/`、`archive/`、`patch/` 等）。
> 原因只有一条：跨分支的账本与指导文档是按**根路径**互相引用的
> （例如 26.2 侧的 `SYNC_GUIDE_REFAB_1211_20260830.md` 直接写 `docs/SCOPE_PIP_RERENDER_…`），
...
| 文件 | 内容 |
|---|---|
| `HANDOFF_LEDGER.md` | 同步账本（本分支副本）：上游指向本分支的行的核对结果 + 本分支新开的行（L-1…L-5） |
| `SYNC_GUIDE_1211_TO_2612_TML_GPU_20260831.md` | 给 26.1.2 的 TML/GPU 移植指导（含 26.1.2 必须先实测的 Q1-Q6） |
...
| `SCOPE_PIP_DEPTH_1211_STEP1/2/3_20260830.md` | 深度版 PIP 三步的实现记录 |
...
`PORT_R2_TO_26_1_2.md`、`PORT_R2_TO_26_2_MAIN.md` —— 把 R2 功能分别移植到 26.1.2 / 26.2(main)
