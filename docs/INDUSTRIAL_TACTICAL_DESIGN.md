...
- 实际成功消耗的工业弹药会在服务端生成可拾取的 `spent_case` 原生掉落物；它必须用匹配弹壳模具经部署器整形后才能重新装填。RPG-7 作为火箭弹显式不抛壳；
...
后续 Phase 4 重点转为拆枪回收、报废模具、弹匣袋/装弹器，以及继续补齐非常规第三方供弹结构；默认枪与默认散装弹已不再留在通用回退链中。
