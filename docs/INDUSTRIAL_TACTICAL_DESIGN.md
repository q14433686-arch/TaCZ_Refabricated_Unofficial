...
- 默认枪包全部 24 种散装弹药的最终四物料装配由带独立槽位的 TACZ 弹药装配机完成；工作盆、置物台/传送带不再被要求推断多种 custom-NBT 输入。无对应默认枪的五种口径以机械合成器制造专属量规，40 mm/RPG 战斗部经顺序部署器装入 TNT；
- 默认枪包全部 53 把枪的模板、机匣/枪身、枪机/套筒、枪管、击发组、复进组件均已作为数据驱动 NBT 部件生产；已校准平台保留专用命名，其余默认枪由内置源定义自动生成独立高保真平台资源；
...
- 新增四格实体弹匣袋和可复用装弹器，前者保留每只弹匣的余弹/兼容族，后者从玩家背包批量转移匹配散装弹；
- 全部工业终端枪均以机匣/枪身为唯一流动过渡工件：蓝图部署器以 `keep_held_item: true` 保留模板，枪机、枪管、击发组、复进组件与外装材料由后续部署器逐站加入；没有任何步骤要求在置物台上同时放多个物品。
