...
| 文件 | 状态 | 内容 |
|---|---|---|
| `compile-check.yml` | **待上线 v4**（正式件是 v3） | 补主分支/PR 编译盲区；日志回推仍只在 arena/**；加 concurrency 取消过期 run |
| `consistency.yml` | **待上线 v2**（正式件是 v1） | 补 arena/** 触发（版本号改动都在工作分支，合并后才守门就晚了）；paths 加 fabric.mod.json |
| `build.yml` | **待上线（新文件）** | 全量 `gradlew build` + jar 上传为 artifact（14 天）——NV 征测/实测直接发 Actions 链接；顺带 mixin 配置完整性、en/zh 语言键齐平、版本一致性三个静态校验 |
