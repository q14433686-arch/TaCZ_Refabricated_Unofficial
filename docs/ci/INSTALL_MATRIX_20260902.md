> **【26.1.2 线副本 · 2026-09-02 抄自 26.2 线 PR #87 `fcd3b4a`】** 家族级文件按副本引入，
> 顶部注明「副本、勿在副本里另立一套」：本文件是**只读参照**，源以 26.2 线为准
> （`q14433686-arch/TaCZ_Refabricated_Unofficial` 分支 `26.2(main)`/PR #87）。
> 本线相关的可执行条目是 **§B1**（四个手动动作，文件都在本线 `docs/ci/`）；
> 文中 `docs/ci/pending/…` 的源文件**只存在于 26.2 线**（renov 三线代拟件、
> refab 1.21.11 补丁），本副本不含它们。
