...
| P0 | 漏夹 `en_bloc_clip` | 仍是 reference-only；没有“入枪、随射击扣除、打空自动弹出”状态 | 独立实现，不能复用桥夹增量转入。 |
| P1 | 工业维护 / 模块维修 | 当前不存在；回收站是破坏性报废，不是维修 | 按本文件 A → B → C 实施。 |
| P1 | 长按 R 供弹器选择圆盘 | 服务器预留已存在，客户端选择 UI 尚未实现 | 在 route/选择槽位协议稳定后实现，不越过服务器事务。 |
| P1 | 速度装填器代表包 | 通用服务端机制已存在，但没有已审计的真实包/动画样本 | 等拿到代表枪包后按同样四层审计接入。 |
| P1 | RPG 发动机壳体 | 文档明确缺独立 ItemStack/NBT stage | 添加真实中间件后才绑定视觉/回收路径。 |
...
| P2 | 枪械等级 | `ModernKineticGunItem#getLevel/getExp/getMaxLevel` 仍返回 0，`ServerMessageLevelUp` 也被封存 | 单独决定是否恢复升级系统；不要与维护值混用。 |
