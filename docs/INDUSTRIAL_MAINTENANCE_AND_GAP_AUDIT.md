...
3. 已新增 Create 单工件维修链：钢/黄铜 → 加热 Basin 形成中性 `service_part_blank` → **同一条传送带**依次经组件专用模具部署器（保留）和 production 模板部署器（保留）校准命名 `service_part` → 损坏组件在另一条**同一条顺序带**依次经检具部署器（保留）、命名维修件部署器（消耗）、动力冲压机替换为同 GunId/同平台、满 Condition 的组件；复装按钮只保留组件 Condition、清洁 Fouling，绝不直接满修；
