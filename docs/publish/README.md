...
| `../UPDATE_REPORT_26_1_2_R1.md` | 通用更新报告 | GitHub Release、群公告或长更新说明 |
...
**不要**用 `TaCZ Fabric 26.1.2 Port` / `TaCZ Refabricated 26.1.2 (Unofficial Port)`
—— 前者含版本号与游戏名，后者含版本号与 filler data，**都会被驳回**。

> 这同时解决了「以后移植到别的 MC 版本会很尴尬」的问题：
> 项目名长期稳定，版本信息由**文件名**（`TACZ-Refabricated-26.1.2-1.1.8+fabric.26.1.2.R1.jar`）
...
| 项 | 值 |
|---|---|
| 模组版本号 | `1.1.8+fabric.26.1.2.R1` |
| Minecraft | `26.1.2` |
| Fabric Loader | `>=0.19.3` |
| Fabric API | `0.155.2+26.1.2` |
| Java | `>=25` |
| 硬依赖 | `forgeconfigapiport >=26.1.5` |
| modid | `tacz`（**不可更改**，枪包依赖此 ID） |
| 内置兼容标识 | `provides: ["lrtactical"]`；无需另装 LR jar |
| ZIP 刀包 | 支持大多数普通、未加密、标准 LR 结构的刀包 |
| 代码许可 | GPL-3.0（含内置 LR 代码移植） |
| 默认枪包资源许可 | CC BY-NC-ND 4.0 |
| LR 原作美术 | All Rights Reserved，不打包、不分发 |

三份文案都必须同时保留三项边界说明：

1. R1 内置的是 **LRTactical 代码兼容层**，支持大多数标准未加密 ZIP 刀包，
   但不是完整 LR 原作资源包；
2. 「需要 TacZ:Arcana 的加密枪包无法加载」——
该问题症状是紫黑块、极易被误判为本移植版的 bug，
且因版本号已对齐 `1.1.8`，这类包反而能通过版本校验、正常建出条目，
迷惑性更强。提前写入描述可省去发布后大量重复答疑。
3. 「专服 REI/JEI 作弊拿取 → 紫黑块 + `item.*` 原始键」——
症状像本移植版 bug，实际是 REI 未安装在服务器时作弊给物退回 `/give` 命令兜底、
丢掉 `minecraft:custom_data` 数据组件所致（REI 源码
`ClientHelperImpl#tryCheatingEntry` 的 `tagMessage` 硬编码为空，
`TODO 24w09a` 组件化后未适配）。
解决：专服安装同版本 REI、用 TACZ 自带创造标签页或内置查询分类拿取、
`/give` 附带组件。该条与「四处 `Item#getName` 改读 common 索引」相互独立。
