...
4. **判别到底是剔面还是明暗**（我们没做，这是接手者的第一步，也是本记录唯一缺的一环）：在第 3 步的状态下
   绕着枪看，有没有「看穿外壳看到内壁」的镂空（准星座、弹匣底、枪口内壁最容易看出来）。
   有镂空 ⇒ 面被剔掉了，下面第 2 层成立；只是明暗不对、无镂空 ⇒ `entityCutout` 其实不剔背面，
   **本节的解释要重做**，§5.7 那张矩阵也要跟着改。
...
2. **消费层**：collector 用 `RenderTypes.entityCutout`（原版里它与 `entityCutoutNoCull` 成对存在 ⇒
   推断前者剔背面；沙箱无 Loom jar，字节码没核实），而自研的两条 GPU 管线显式 `.withCull(false)`。
   ⇒ **同一段几何在两条路上的「绕序敏感度」不同**：GPU 路上绕序只影响 facing（观感细微，所以第 0-3 轮
   实机没看出来），collector 路上它决定「这个面画不画」（观感致命）。这也解释了为什么第 5 轮把光影下的
   GPU 键退回 false、高模回到 collector 之后，这个问题才第一次显形。
