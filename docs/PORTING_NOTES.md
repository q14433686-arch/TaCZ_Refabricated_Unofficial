...
### 3.5 【2026-08-10 已修正】镜内裁剪区域：从「目镜几何投影」改为「屏幕空间圆」

> 本节原为「已知偏差，尚未修正」，2026-08-10 已实现修正（见文末「修正记录」），
> 保留原分析作为历史依据。

这是本移植与上游的一处**架构级分歧**（历史），记录在此以免重复排查。
...
> ### ✅ 修正记录（2026-08-10）
>
> 按「圆」语义重新实现并保留至今，与上次失败的差别：
>
> 1. **圆心**：取目镜几何在掩码空间（视图空间）的**投影质心**，而不是
>    屏幕中心，也不是 `Camera#getViewRotationProjectionMatrix` 的错误空间。
>    质心与半径都来自当帧登记的那份 `pose`（`RenderSystem.getModelViewMatrixCopy()
>    × poseStack` 烘焙矩阵，与几何投影方案同一套、已被 33 个瞄具验证过位置正确）。
> 2. **半径**：按目镜几何到质心的最大 XY 外延**自标定**（× `SCOPE_CIRCLE_RATIO`
>    = 0.9 × 开镜进度），不再用 `80/(16·90) ≈ 0.0556 格` 这种与模型无关的常数。
>    于是默认枪包（实心盘）与第三方 PU 镜（6 辐条）都能得到实心圆。
> 3. **拓扑**：掩码管线 QUADS → TRIANGLES，直接写三角形扇
>    （上次用「退化四边形绕开 QUADS」属于绕路，多边形扇更直接）。
> 4. **开镜进度**：半径已编码进度（上游 `rad = 80·progress` 语义），
>    绿通道写 1.0，让 shader 里那套「屏幕空间收缩」休眠 —— 避免双重收缩。
> 5. **附带收益**：镜筒黑环恢复（目镜黑片画在圆外）；
>    开镜用筒镜时整枪/配件也走掩码裁剪（`GunItemRendererWrapper` 前置判定 +
>    静态强制标志），镜内不再透出枪体/枪管/枪口装置。
>
> 回退开关不变：`RenderConfig.SCOPE_MASK_ENABLE`。黑环宽窄调
> `ScopeMaskRenderer.SCOPE_CIRCLE_RATIO`（越小环越宽）。
