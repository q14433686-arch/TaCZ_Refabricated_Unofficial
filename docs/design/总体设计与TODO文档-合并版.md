...
|---|---|---|---|---|---|---|---|
| A-1 | 材料树 | 02-A | P1 | ✅数据层完成 | — | 2026-08-01 | MaterialType+24 JSON+Registry；实机编译绿 @f189908 |
| A-2 | 手工阶段(热度条锻打) | 02-A | P1 | ✅数据层完成 | — | 2026-08-01 | HeatBand/HeatWorkData组件/3冷却曲线/4工序/规则层；方块与GUI后置 |
| A-3 | 小作坊机器 | 02-A | P1 | ⏳待物品层批次 | — | 2026-07-31 | mold/barrel_part 组件设计已定型，待物品开门 |
| A-4 | 初级工业动力机 | 02-A | P5 | 未开始 | — | | 依赖 Q-02 |
| A-5 | 中级工业化学线 | 02-A | P5 | 未开始 | — | | |
| A-6 | 重度自动化双路线 | 02-A | P5 | 未开始 | — | | |
| A-7 | 蓝图/图纸 | 02-A | P5(前置字段 P1) | 设计中 | — | | 依赖 Q-03 |
| A-8 | 公差系统 | 02-A | P1 | ✅数据层完成 | — | 2026-08-01 | 三表 JSON+ToleranceRules 全公式（含事故报告明细）;Q-04 关联待 C 注入 |
...
| 2026-08-01 | Q-21 关闭：CI 编译闭环建成（四层日志回推），三轮实机排错（66→42→0）compileJava BUILD SUCCESSFUL；Gradle 9.5.1 Groovy 类型化闭包 NPE 等 8 层成因入档 |
| 2026-08-01 | P1 数据层+规则层落地（17 新类 + 43 数据 JSON）：A-1 材料树（24 材料）、A-2 热度条（HeatBand/workpiece 组件/冷却曲线×3/T1 工序×4/HeatRules/ProcessRules/QualityRules）、A-8 公差系统（机器 TS×6/分级带×5/装配权重/ToleranceRules 含事故报告明细）；loader 重构通用多目录；实机编译绿 @f189908（实现记录 docs/impl-log/P1-manufacturing-foundation-data-layer.md）。后续批次：B 弹药四要素 → C 注入最小闭环（前置 Q-06 最高优先级 Spike）→ 机器物品层 |
