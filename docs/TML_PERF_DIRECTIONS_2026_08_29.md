...
> **状态更新（2026-08-30，全部实机 PASS）**：本文写作时主分支尚无 TML 代码；
> 现状已大幅前进，按 §2 的落地顺序：
> - **第 0 步（安全子集）**：完成（`8c6ad27`）;
> - **第 1 步（无光影 GPU 路径）**：完成（`8191f6b` + `0ea0fb6` 两层变换修复）;
> - **第 2 步 / 方向 1（光影下 GPU）**：完成，但**不是**本文推测的自建
>   `mesh_entity_hand` 管线路线——实际走「vanilla RenderType 路线」
>   （`6e275d0`：常驻 VBO 经 `RenderType.prepare()`+`drawFromBuffer` 用
>   ENTITY_CUTOUT 管线绘制，Iris 按既有 HAND 接管链路照明），
>   比 PoC 方案更省一条自建管线且照明语义与立方体逐位一致;
>   后续补了光影开关切换时的烘焙世代失效（`9f7412e`，修站桩切光影拉伸）;
> - **方向 3.1（PIP×mesh 放大器）**：完成（`4fb137d` 镜内那遍跳过 poly 提交）;
