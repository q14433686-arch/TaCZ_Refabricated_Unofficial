...
#### 案例⑦：带模型的「炮弹」与「炮烟」（弹药尾烟）第一人称从眼睛飞出 —— 已修（第 31 轮）

- **症状**：用户实测：带弹药实体模型的弹种（炮弹/榴弹等）与其尾烟，
  飞行起点与轨迹仍在**眼睛处**——第 26 轮的曳光枪口锚定没有覆盖它们。
  用户原话即诊断：「炮弹、炮烟的模型是特殊的吗？」——**是的，是特殊路径。**
- **根因（结构性，三条视觉载体曳光修复只治其一）**：
  1. 曳光条带（`renderTracerAmmo`）—— 第 26 轮已锚定枪口；
  2. **弹药实体模型路径**（`EntityBulletRenderer#submit` 内的
     `ammoEntityModel.submit(...)`）直接用实体位置+朝向绘制，
     【没有任何第一人称锚定】；
  3. **弹药尾烟**（`AmmoParticleSpawner.addParticle`，由
     `EntityKineticBullet#tick` 每 tick 客户端触发）在实体当前位置播粒子；
     散点式（count>0）有「距发射者 3 格内不生成」抑制，但 count==0 的
     单颗粒轨迹【连抑制都没有】。
  而 `EntityKineticBullet` 服务端出生在射者眼位（标准 FPS 弹道起点，
  命中判定正确无虞）——后两条载体于是从「眼睛」出发。
- **修复（同一条数学链，零新意=零风险）**：新增统一锚定入口
  `EntityBulletRenderer#firstPersonMuzzleAnchor(bullet, bulletPos, eyePos)`：
  视图空间 `muzzleRenderOffset` × 当帧相机旋转（视图→世界，直接 rotate 不取
  共轭）→ 世界轴偏移，再按弹眼距 `(50−d)/50` 线性收敛 —— 出生瞬间整块
  位移到枪口，50 格外精确回到真实弹道。**模型路径与尾烟路径共用同一函数**，
  保证两者贴着同一条「从枪口喷出」的可见轨迹。
  - 仅【本地玩家本人第一人称】返回非 null；第三人称自己/旁观他人
    保持原生实体位置（眼位≈持枪手区域，与上游观感一致）；
  - 新开关 `FirstPersonAmmoMuzzleAnchor`（默认 true；置 false 立即回退）；
  - 附带治理：模型路径补了一层 `pushPose/popPose` —— 原实现的两条
    YP/XP `mulPose` 会泄漏给紧随其后的曳光段（模型+曳光并存的弹种
    曳光会被二倍旋转，潜在 bug）。
- **第 31.2 轮修正（用户实测反馈：炮弹/炮烟「固定在枪口上方」）**：
  初版照抄了曳光的 **50 格收敛窗口**。曳光弹速 ~800m/s、0.06s 收敛完毕
  不可感知；但炮弹/榴弹是低速长寿命弹种，会在窗口内飞行数秒，全程背着
  「跟随当帧相机旋转、随距离缓慢缩小」的幽灵位移 → 转头时炮弹跟着画面甩、
  像被**钉在枪口上方**。修正：本路径收敛窗口缩到 **2.5 格**（出膛瞬间即从
  枪口并入真实弹道；窗口内残余跳变几乎都是沿视轴的前向分量，屏幕上不可见）。
- **第 31.3 轮修正（用户两张截图实证：起点「视角内偏上」）**：
  面南/面西截图一致——起点稳定偏上 = 确定性符号错误。根因：存量
  `muzzleRenderOffset` 的捕获链贯穿床岩视模渲染（y 翻转惯例），其纵向
  分量对真实视图空间反号。曳光共用同一向量却从未暴露（5mm 宽、800m/s、
  0.06s 收敛，~0.19 格镜像不可见）；炮弹/烟又大又慢立刻现形。
  修正：锚定函数内在 rotate 成世界轴【之前】把视图偏移的 y 分量取反
  （x 不动：+0.16 指向屏幕右方，与右下持枪视模相符）。
  曳光已验收行为不动；若要全局统一应回到 `cacheMuzzlePosition` 的捕获
  空间规范解决，勿顺手改曳光。
- **验证**：带弹药模型/尾烟的弹种（榴弹/炮弹类枪包），第一人称开火：
  炮弹与烟应从枪口喷出并**立刻**并入真实弹道（不再是长时间挂在一旁）；
  第三人称与旁观不变；曳光弹（无模型弹种）行为不变。
