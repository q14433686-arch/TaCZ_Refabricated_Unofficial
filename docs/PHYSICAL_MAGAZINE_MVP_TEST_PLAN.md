...
- M870、SPAS-12、M1014：一次完整的脚本换弹循环可按剩余容量逐发装满管式供弹；服务器必须从散装弹堆精确扣除同样数量，不能出现可退卸弹匣；
- Rhino 357、Taurus 500、Taurus 943：一次完整转轮换弹循环按剩余容量装填，散装弹数量与枪内转轮状态必须逐发一致；
- RPG-7、M320、双管枪：单发/双管后膛状态不接受实体弹匣；双管枪空仓一次换弹必须实际扣除并装入两发 12G，而不是只保留一发；
