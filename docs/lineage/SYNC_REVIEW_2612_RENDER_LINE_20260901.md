...
| G5 光影下二次渲染隔离大件 | `3e8b22e` `1c2c5b5` `825d2c5` `95590b0` `82b3262` `c42b047` `2027261` `7b4a9a2` `3d8432f` `d3f0fdc` | Iris scope-pipeline 隔离（dimension id + 预热）、`ScopePipIsolatePipeline` 配置、ShadowScale、idle release 熔断、Voxy 第二渲染栈 + reload 钩子、Sodium 私有投影快照同步、ESC 崩溃三道闸 | **不移植（本世代前提不同）** | 理由：我方**没有** `compat/voxy`、`compat/sodium`（实读 MISSING），且我方 `ScopePipRerender:150` 仍是他们 `825d2c5` **删掉之前**的 B1 硬拒（`isUsingRenderPack()` ⇒ `return false`）⇒ 光影下窄遍根本不跑，他们这批要治的三症状（整屏拖影/云噪点/镜外发糙）在我方无从出现。若将来要放行光影窄遍，**这批必须与放行同时上**，否则等于把我方推进他们已修的坑 |
