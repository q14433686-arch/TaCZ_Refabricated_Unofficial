...
那类问题带到默认路径 ⇒ 我们当时退回 false（`9c29572`，账本 L-6/复核篇 §8）；**2026-09-02 我方已翻回
默认 true，与 `3e4eeb16` 对齐**（§5.10 照明不等价保留为 A/B 键：个别包若再现，把两键一起关掉即回 collector）。
本条原先"等实测数据再重评"的开放项随之关闭。
