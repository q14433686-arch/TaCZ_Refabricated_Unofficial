...
| 14 | 本仓 `docs/lineage/SYNC_GUIDE_PUTAWAY_KEEP_20260902.md`（收枪动画 `keep()` 修复的三线移植） | 外部 fork `ca2b9fc` → refab 26.2 / 26.1.2 / 1.21.11 | **26.2 已落码（待实测）；26.1.2 + 1.21.11 = OPEN** | 无共同 git 祖先，不能 cherry-pick；`LocalPlayerDraw.java` 三线同 blob `79887cf`，机制（`KeepingItemRenderer`/`getMainRenderStack`/mixin `keep()`）三线逐字同构 ⇒ 属「同代码同机制」移植。两份补丁 `docs/patch/2026-09-02-putaway-keep-render-{26.1.2,1.21.11}.patch` 已对真实 blob `git apply --check` 干跑通过；1.21.11 不新增 mixin 目标，refmap 无涉。**实测项见指导 §5（开镜消费者分属两纪元，需各自 A/B）** |
