...
| # | 事项 | 方向 | 状态 | 备注 |
|---|---|---|---|---|
| L-1 | `docs/lineage/SYNC_GUIDE_1211_TO_2612_TML_GPU_20260831.md`：内置 TML 第 0/1/2/3 步整包 + 五条不变量 + 26.1.2 需先实测的 Q1-Q6 | 1.21.11 → 26.1.2（`arena/01a05170`） | **OPEN** | 26.1.2 目前 meshloader 文件数为 0（`git ls-tree` 核实），所以是「先第 0 步、再往上叠」，不是打 GPU 补丁。要求回给：Q1-Q6 实测答案 + 世界路径两条主验收项的实机结论 |
