...
| L-10 | 镜内 `text_show`（MK5/MK5HD 弹药计数）**从不显示**：`functionalTasks` 在瞄具深度孔径路径上没被 flush。根因与修法由 26.1.2 给出，本分支已落地（三个文件） | 26.1.2 → 1.21.11（收）；回礼 2 条给 26.1.2（26.2 无此洞） | **代码已改，实机未验**（剧本见 `docs/lineage/SCOPE_TEXT_SHOW_1211_20260901.md` §4 的四格） | 静态闭合三条：`submitFunctionalTasks` 全仓仅 `BedrockModel.java:381` 一处且瞄具不走 `super.submit`；26.2 第 681 行确有 `super.submit` ⇒「26.2 没这洞」我方独立确认；本仓无 `ScopeTextSubmitter`/`scope_text.*sh` ⇒ 26.2 的 `9d036594` 是裁剪不是存在性。回礼差别①：他们把 `else` 分支的 flush 放在 `!bodySnapshot.isEmpty()` 门内 ⇒「只有文字没有本体几何」的快照仍会丢；差别②：`ocularRingSnapshot` 的任务他们没 flush。**延迟覆盖层那一格文字不测镜筒深度 ⇒ 贴边计数仍可能溢出，真要裁需移植 26.2 那套**（本分支有意不做） |
