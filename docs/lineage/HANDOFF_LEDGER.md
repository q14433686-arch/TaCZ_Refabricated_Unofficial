...
| M-12 | 六线 CI 盘点 → 本线上线就绪：三件暂存稿核对（与 26.2 线逐字一致、仅头部注释差异）+ 静态检查待办并入 `docs/ci/compile-check.yml`（v4）+ 本线 `docs/ci/README.md` 刷新为四步上线清单 + `AGENTS.md` §1 模板路径修正（`docs/publish/ci/` → `docs/ci/`）+ 家族级 `INSTALL_MATRIX_20260902.md` 按副本引入 + **mesh parity 红线修正**（M-7 面板条目的语言键名未跟随 toml 键蛇形，`mesh_gpu_bake_budget`→`mesh_gpu_bake_budget_per_frame`，6 处改名：Java 2 处 + en_us 2 处 + zh_cn 2 处，显示文本不变） | 26.2 线 PR #87 `fcd3b4a`（`docs/ci/INSTALL_MATRIX_20260902.md` §B1）→ 26.1.2 | **暂存就绪，待维护者网页端上线** | 四步：① 建 `build.yml` ② 建 `consistency.yml` ③ 建 `compile-check.yml`（v4+三静态检查）④ **删** `compile-check-2612.yml` —— ③④ 必须一次做完（同 `name:` 双跑）。**首跑预期绿（本会话已在本工作区实跑三静态检查全绿）**；注意 26.2 线矩阵的预演清单只含 mixin/lang/一致性脚本，**没含 mesh parity**——其「首跑预期绿」若在三检查并入状态下成立，前提正是本次这个改名（不改名则 `check_mesh_config_parity.py` 在本线 rc=1，首跑必红）；**真实 Actions 首跑未验证**。本线装完 build.yml 后即有 jar 产物（六线里仅 refab 26.2 有）；consistency 上线前，本线改版本号不同步 README 不会红 |
