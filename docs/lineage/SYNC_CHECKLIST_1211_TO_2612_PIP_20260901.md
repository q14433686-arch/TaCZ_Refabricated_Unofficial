# 给 26.1.2 的同步清单（镜内文字裁剪 PASS 之后 + 我方 31 个真实提交的剩余确认项）
...
| 你们 `58831e4f`（FCAP `save()` no-op ⇒ 配置每次重启被重置）在 1.21.11 **不适用** | 我方 `ConfigValue.set()` 走的是带 autosave 的旧文件配置链，且你们自己的文档 §0 写了"1.21.11、26.2 都没有这个问题" | 请把它当**世代边界**而非通用缺陷；我们不会跟改，也请别让后人把 `ConfigPersist` 搬过来 | 静态（引你们文档 §0 的三分支逐字比对）+ 我方读码 |
