...
| `9c29572`/`ab11a84`（绕序默认退回 + 立项） | 规避有效；绕序×剔除不自洽本身不修（L-8b）。**「反光/高光偏一侧」另有已确认根因**（26.2 `83daf16`）：光影包的 `gl_NormalMatrix` 由 Iris 在**绘制执行那一刻**读 RenderSystem MV 栈顶的逆转置，不吃 DynamicTransforms 快照 ⇒ 与我方 GPU pass「从不压栈」同病，我方已同修（实机待验） | **非项**：你们 `meshloader`/`PolyMesh` 0 命中 ⇒ 既没有自建 pass，也没有「骨骼本地法线裸写 + 依赖绘制当刻矩阵」这条链 |
| `5ac0262`/`25ca08e`（EMISSIVE 永久降级修复 + 光影两键默认关） | 两键仍在、每帧读、不需重启；与 Fabric 26.1.2 的默认值选择相反（他们 `3e4eeb16` 开 ON） | 非项 |
| 对 26.1.2 线本轮 PIP 回移植的**评估结论**（被评估的是**他们**的提交 `0a77ef52`…`8aca7374`；`18e4553`…`55ad5e2` 是我方**写评估**用的提交。我方线上没跟做 PIP 重渲染，`SCOPE_PIP_RERENDER` 仍锁 false） | A 结案（不加）；B 已 PASS；C 待批；他们的双遍风险仍待实机 | 见 §5 头两行 |
