...
即时“右键整叠填满”与即时“清空一整只弹匣”的旧路径必须移除，不能成为绕过时间的后门。新的**弹药处理台**是独立的真实服务端多槽事务：

```text
[实体载具] [弹药输入 A] [弹药输入 B] [弹药输入 C] [弹药输入 D]
[装弹器工具] [退弹输出 A] [退弹输出 B] [退弹输出 C] [退弹输出 D]
```

- 载具槽接受经过审计的实体弹匣、桥夹、漏夹和快装器；不会把未知 legacy 枪强制变成实体供弹。
- 每次“装一发”只在计时结束后从选定输入槽扣除**一发**并压入载具顶部。
- “连续装填/连续退弹”只是重复同一个服务器逐发事务；每一发都独立等待、校验、扣除/输出，可随时因载具移动、输入耗尽、输出堵塞、关闭 GUI、离开距离或服务端 profile 关闭而停止。
- 退弹每次只弹出顶部一发到匹配的输出槽；输出无空间时整次操作停在原状，绝不丢弹或改写为另一种 AmmoId。
- 创造/服务器免费弹药规则仍保留处理时间，但不扣选定输入槽；玩家仍必须放入一份真实 profile 样本来声明要装哪种弹。
- 装弹器只能作为处理台中的真实工具槽影响速度，不能恢复旧的瞬时批量装填。
...
| 退出一发 | 8 tick / 0.40 s | 顶部逐发退弹；输出槽不足即暂停 |
| 装弹器工具 | `×0.75`，最低 4 tick | 仍是一发一次的真实事务，不是批量瞬移 |
...
- 已审计的桥夹/漏夹仍必须沿用各自真实脚本/动画 feed 点；处理台只改变它们背包内的物理内容，不伪造装入枪内的动画。
...
1. 有序载具内容与旧世界无损迁移；
2. 真实多槽处理台逐发装/退、取消、满输出、重连与背包/GUI移动校验；
