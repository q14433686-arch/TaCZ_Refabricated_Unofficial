...
| **是** | **Beta-3** 测试构建：已可游玩、可公开测试，但仍不保证无 bug |
...
Beta-3 在 Beta-2 的可游玩基线上，按 `26.2(main)` 的 `backport-26.1.2/` 清单回移植了
7 项已在 26.2 验证的修复，并通过签名适配移植补齐了工作台预览模型的缩放/旋转。
它仍然是测试构建，不保证与上游完全等价。

Beta-3 主要新增 / 修复（回移植自 26.2(main)，逐项明细见
...
>
> 本移植版的版本号是 **`1.1.8+fabric.26.1.2.Beta-3`** —— 前面的 `1.1.8` 是所基于的
