...
直接上游的版本号为 `0.7.0-forge1.1.8-hotfix`；本分支当前源码版本为 **`1.1.8+fabric.1.21.11.R3-hotfix`**。
...
| Fabric API | **0.141.6+**；R3-hotfix 构建使用 **0.141.6+1.21.11** |
| Forge Config API Port | **21.11.1+，硬依赖** |
| 本 mod | **`1.1.8+fabric.1.21.11.R3-hotfix`** |
...
作为 SemVer 核心，`+fabric.1.21.11.R3-hotfix` 是构建元数据，不参与 Fabric 的版本先后比较。
