...
public class TaCZFabricClient implements ClientModInitializer {
    ...
    private void subscribeEvents() {
        ...
        //
        // 【必须挂 END，不能挂 START】原版 Minecraft#tick 先处理输入
        // （handleKeybinds → startUseItem），再 tick 世界/实体，而「使用结束」
        // 发生在实体 tick 里。挂 END 才能在【同一次】tick 内采到下降沿；
        // 下一次 handleKeybinds 已是下一个 tick，MinecraftUseRestartMixin
        // 因此拦得住。完整病因见 UsePressGate 类注释。
        ClientTickEvents.END_CLIENT_TICK.register(
