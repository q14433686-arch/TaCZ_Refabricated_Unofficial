 ...
 *
 * <h2>证据链（2026-09-01 实机日志 + TaCZPackUpgrader 源码实读）</h2>
 * <ol>
 ...
 * {@code tacz:nbt} 的那批全灭。
 *
 * <p>本版本按 <b>1.21.11 Fabric API</b> 的目标接口实现：与同包
 * {@link PartialNBTIngredient} 一致使用 {@code getMatchingItems()}、
 * {@code toDisplay()}、{@code getPacketCodec()}（26.2 的目标接口是
 * {@code items()}/{@code display()}/{@code getStreamCodec()}，两者不可互抄）。
 *
 * <h2>语义</h2>
 ...
 * {@code GunSmithTableIngredient.normalizeLegacyIngredientJson} 负责，本类 codec 只认数组。
 */
public class TaczNbtIngredient implements CustomIngredient {
     ...
     * 而不是裸基础物品 —— 与 {@link PartialNBTIngredient#toDisplay()} 同一理由。
     */
    @Override
    ...
    public static class Serializer implements CustomIngredientSerializer<TaczNbtIngredient> {
        ...
        @Override
        public StreamCodec<RegistryFriendlyByteBuf, TaczNbtIngredient> getPacketCodec() {
