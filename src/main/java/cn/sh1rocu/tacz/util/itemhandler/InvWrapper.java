...
public class InvWrapper implements IItemHandlerModifiable {
    ...
    public CompoundTag serializeNBT() {
        ...
        for (int i = 0; i < getSlots(); i++) {
            if (!getStackInSlot(i).isEmpty()) {
                CompoundTag itemTag = (CompoundTag) ItemStack.CODEC.encodeStart(NbtOps.INSTANCE, getStackInSlot(i)).getOrThrow();
    ...
    public void deserializeNBT(CompoundTag nbt) {
        ...
        for (int i = 0; i < tagList.size(); i++) {
            ...
            if (slot >= 0 && slot < size) {
                setStackInSlot(slot, ItemStack.CODEC.parse(NbtOps.INSTANCE, itemTags).result().orElse(ItemStack.EMPTY));
            }
        }
    }
}
