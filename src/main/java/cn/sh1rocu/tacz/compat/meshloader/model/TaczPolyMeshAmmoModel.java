...
/**
 * 支持 poly_mesh 的弹药模型。移植自 VellEagle/TacZMeshLoader 1.21.1_fabric (GPL-3.0)。
 */
    ...
    public void loadPolyMesh(Identifier modelLocation, Identifier textureLocation) {
        try {
            ...
            if (this.polyMeshModel != null && MeshyConfig.LOG_STATS.get()
                    && PolyMeshSupport.markGeoLogged(modelLocation)) {
                LOGGER.info("[TacZMeshLoader] ammo poly_mesh stats for {}: {} bones, {} vertices",
                        modelLocation, polyMeshModel.getMeshBoneCount(), polyMeshModel.getTotalVertexCount());
            }
        } catch (Exception e) {
