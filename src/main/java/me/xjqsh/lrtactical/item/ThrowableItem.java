...
public class ThrowableItem extends Item implements IThrowable, com.tacz.guns.api.item.IAnimationItem, cn.sh1rocu.tacz.api.extension.IItem {
     ...
     *
     * <p>更早的一版只在服务端查冷却、客户端一律放行，当时的理由是
     * 「客户端表可能因包延迟仍为空/已过期」。那种写法本身就会分叉：
     * <ul>
     *   <li><b>服务端</b>：冷却中 → 不 {@code startUsingItem}；</li>
     *   <li><b>客户端</b>：照常 {@code startUsingItem}，进入“使用中”。</li>
     * </ul>
     *
     ...
     * </ul>
     *
     * <p>客户端那份表为什么可以当门禁（本分支逐条核对，不是照搬 26.2 结论）：</p>
     * <ol>
     ...
     *       会再发一条 {@code duration=0} 的消息（{@code CustomItemCoolDowns}），
     *       客户端立刻 {@code removeCooldown}，两边对齐，多拒窗口≈一个单向延迟。</li>
     * </ol>
     *
