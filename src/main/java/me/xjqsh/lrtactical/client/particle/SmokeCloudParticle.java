...
public class SmokeCloudParticle extends SingleQuadParticle {
     ...
     * <h2>26.2 差异（字节码确认）</h2>
     * <ul>
     *   <li>方法名是 {@code getLightCoords(float)}，不是 1.21.1 的 {@code getLightColor(float)}；
     *       在 {@code Particle} 上为 <b>protected</b>，这里保持同样可见性。</li>
     *   <li>不走 {@code LevelRenderer.getLightColor}（签名已迁走），
     *       改用 {@code BlockAndLightGetter#getBrightness(LightLayer, BlockPos)}
     *       自己打包成 {@code sky << 20 | block << 4}，与原版
     *       {@code LightTexture} 的编码一致。</li>
     * </ul>
     */
    @Override
