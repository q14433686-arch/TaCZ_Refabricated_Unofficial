 ...
 *
 * <h2>不能直接复用 {@code ItemTransform#apply}</h2>
 * 26.x（本分支 26.1.2 与 26.2 均如此，26.1.2 已对
 * {@code ItemTransform#apply(Z, Pose$Pose)} 的字节码逐一确认）内部自带
 * 两次 {@code translate(±0.5, ±0.5, ±0.5)}（本仓
 * {@code MeleeItemRenderer#renderByItem} 的注释已就同一点踩过坑）。
 * 那个半格偏移是为「物品在方块格内居中」准备的，拿来摆飞行实体会整体偏半格，
 * 所以这里按 1.20.1 {@code ItemTransform#apply(false, poseStack)} 的语义
 * 手写平移 → 旋转 → 缩放三步。
 */
public final class DisplayTransform {
     ...
     * {@link MeleeDisplayInstance} 的类注释（26.2 的 {@code ItemTransform}
     * 反序列化器构造器已降为包级私有，外部包无法实例化）。
     *
