 ...
 *
 * <h2>它治的是什么病</h2>
 * 拿着有使用时长（进度条）的 LR 物品（手雷 / 闪光弹 / 消耗品）长按右键，
 * 物品用完之后如果还不松手，进度条会<b>再读一次</b>、动作也会重来一遍，
 * 但物品不会被消耗；而且只要不松手，姿势与进度就一直<b>卡在最末尾</b>。
 *
 * <h2>根因是原版行为，不是我们哪一行写错了</h2>
 * 原版 {@code Minecraft#handleKeybinds} 在右键按住期间每 tick 都会进：
 * {@code keyUse.isDown()} 且 {@code rightClickDelay == 0} 且 {@code !player.isUsingItem()}
 * 时调用 {@code startUseItem()}。本分支已交叉确认该方法名：
 * {@code MinecraftMixin} 注入的就是 {@code startUseItem}，
 * {@code InteractKey} 也直接调 {@code Minecraft#startUseItem()}。
 *
 * <p>也就是说：<b>使用一结束，下一个 tick 的原版输入循环就会立刻再开一次</b>。
 ...
 * {@code MultiPlayerGameMode#useItem} 把 {@code ServerboundUseItemPacket}
 * 放在 {@code startPrediction} 的回调里构造，<b>先于</b> {@code ItemStack#use} 调用 ——
 * 所以只在 {@code use} 里返回 FAIL 拦不住包：服务端照样会被问一次，
 * 而服务端没有「这次按压已经用过了」的概念，于是服务端 {@code startUsingItem}、
 * 客户端没有 → 换一个方向的分叉。
 * 拦在 {@code startUseItem} 之前则两头都干净：本地不进入使用状态，包也压根不发。
 *
 * <h2>时序为什么可靠</h2>
 * 原版 {@code Minecraft#tick} 先处理输入（{@code handleKeybinds} → {@code startUseItem}），
 * 再 tick 世界/实体；「使用结束」发生在实体 tick 里（服务端停用时靠同步的
 * {@code DATA_LIVING_ENTITY_FLAGS} 落地）。所以本类挂在
 * {@code ClientTickEvents.END_CLIENT_TICK}（{@code Minecraft#tick} 末尾）时，
...
public final class UsePressGate {
    ...
    /**
     * 每客户端 tick 末尾调用（{@code ClientTickEvents.END_CLIENT_TICK}）。
     *
    ...
    public static void onClientTick(Minecraft mc) {
        ...
        if (player.isUsingItem()) {
            ...
            //      比较用 getItem() == 而不是 ItemStack#is(...)：本分支虽有 is(Item)
            //      （ThrowableItem 里已经在用），但这里要比的是「是不是刚才那一件」，
            //      直接比 Item 引用最省事，也不依赖 Holder 包装。
