...
public class GunPackList extends ContainerObjectSelectionList<GunPackList.Entry> {
    ...
    public static class Checkbox extends AbstractButton {
        private static final Identifier TEXTURE = Identifier.withDefaultNamespace("textures/gui/checkbox.png");
        ...
        protected void extractContents(GuiGraphicsExtractor pGuiGraphics, int pMouseX, int pMouseY, float pPartialTick) {
            ...
            pGuiGraphics.blit(RenderPipelines.GUI_TEXTURED, TEXTURE, this.getX(), this.getY(), this.isFocused() ? 10.0F : 0.0F, this.selected ? 10.0F : 0.0F, 10, 10, 32, 32);
            if (this.showLabel) {
                pGuiGraphics.text(font, this.getMessage(), this.getX() + 24, this.getY() + (this.height - 8) / 2, 14737632);
            }
        }
    }
}
