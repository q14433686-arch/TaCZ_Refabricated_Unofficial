 ...
 * metadata). See the Step 2 handoff doc for the exact risk and the fallback plan.
 */
        ...
        if (pipeline == null) {
            ...
            pipeline = RenderPipelines.register(
                    RenderPipeline.builder()
                            ...
                            .withoutBlend()
                            .withColorWrite(true)
                            // Deliberately do NOT call withDepthTestFunction/withDepthWrite:
                            // an untouched depth state keeps this pipeline from requesting a depth
                            // attachment, so reticle depth semantics are not perturbed.
                            .build());
        }
        return pipeline;
    }
