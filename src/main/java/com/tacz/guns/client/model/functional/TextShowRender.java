...
public class TextShowRender implements IFunctionalSubmitter {
     ...
     * 挂在瞄具 ocular 子树上的文字（弹药计数等）传 true：flush 时走
     * {@link com.tacz.guns.client.render.scope.ScopeTextSubmitter} 的孔径深度掩码管线，
     * 溢出圆孔的像素被丢弃；枪身文字保持 false —— vanilla 管线按场景内容正常处理。
     */
    ...
    public void extract(ExtractionContext context) {
        ...
        int packedLight = LightTexture.pack(textShow.getTextLight(), textShow.getTextLight());
        ...
        // 与 26.2/26.1.2 相同的失败语义：不丢字、不画错，最差回到「边缘可能溢出」的旧行为。
