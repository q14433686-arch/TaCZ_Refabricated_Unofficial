...
public final class PhysicalMagazineService {
    ...
    public static boolean isCompatible(GunFeedDefinition definition, ItemStack magazine) {
        ...
        Identifier definitionStandard = FeedInterfaceStandardService.getStandardId(definition);
        if (magazine.getItem() instanceof MagazineItemDataAccessor accessor) {
            ...
            // A manufactured standard carrier records its standard id. An old
            // carrier without that tag is still accepted only through the same
            // audited family/cartridge-standard fallback below; it cannot claim
            // an unrelated standard merely by matching a loose calibre name.
            Identifier carrierStandard = accessor.getFeedStandardId(magazine);
            if (definitionStandard != null && carrierStandard != null && !definitionStandard.equals(carrierStandard)) {
                return false;
            }
        }
    ...
    /**
     * An explicit FeedInterfaceStandard is the cross-platform physical parent
     * contract. A receiver may therefore accept a larger carrier declared by
     * another loaded receiver with the same standard (for example a 20-round
     * M16A1 and a 30-round M4A1/SCAR-L STANAG). Unregistered legacy families
     * remain exact-family/exact-native-AmmoId only. Neither a shared calibre
     * nor a matching GunIndex/model/capacity can create a standard contract.
     */
